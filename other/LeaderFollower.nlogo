...
to setupHongrois
 let m matrix:make-identity taille;
 set i 0;
 set j 0;
  ;remplissage matrice de distance
  while[i < taille][
    ;foreach [robots][ r ->
    let r myself;
    while[j < taille][
        ask points with[id = j][
          ;foreach [points][ p ->
          let d 0;
          set d [distance r] of myself;
          matrix:set m i j d;
          set j j + 1;
       ]
   ]
...
to setup
  ...
  ; creation de la forme (shape)
   ifelse (forms = "carre") [
    create-points 2 [set shape "square"  set assigned false set id who set color white setxy (who * scale) (who * scale)]
    create-points 1 [set shape "square"  set assigned false set id 2 set color white setxy (scale) (0)]
    create-points 1 [set shape "square"  set assigned false set id 3 set color white setxy (0) (scale)]
  ...
  [
    create-points 3 [set shape "circle"  set assigned false set id who set color white setxy (who * scale) (who * scale)]
    ; creation des robots
