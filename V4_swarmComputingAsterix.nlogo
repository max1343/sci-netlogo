breed [robots robot]
breed [points point]
robots-own [assigned leader npoint pointx pointy ciblex cibley voisins grade vs]
...
; voisins       : indique le nombre de voisins trouvés par le robot
...
;chaque robot choisit un point
to init-swarmComputing
...
; 2 robots ont choisi le meme point
to fight
  let opponent one-of robots-here with [vs = false]
  ask opponent [set vs true]

  let mine one-of robots-here with [vs = false]
  ask mine [set vs true]

  if opponent != nobody
  [
    ifelse [grade] of opponent < [grade] of mine
    [
      ask opponent [
        let p one-of points
        set pointx ([xcor] of p)
  ...
  ask opponent [set vs false]
  ask mine [set vs false]
end

to fight2
  ask robots [ let mini min-one-of robots [distance myself]
    if (mini < 2) ; deux robots rentrent en conflit
    [
      fight
    ]
  ]
...
;les robots cherchent leur voisin
to look-for-neighbour
...
to setup
  ...
  ; creation de la forme (shape)
   if (forms = "carre") [
    ...
    ; creation des robots
    create-robots 4 [setxy random-pxcor random-pycor set color white set leader false set assigned false set voisins 0 set grade (random 7)]
   ...
   if (forms = "triangle") [
    ...
    create-robots 3 [setxy random-pxcor random-pycor set leader false set color white set assigned false set voisins 0 set grade (random 7)]
    ask robots [set label who]
  ]
  init-swarmComputing
  choix
...
to go
  ...
  ask robots [ ifelse ((distancexy ciblex cibley) > 0.5) [fd speed][setxy ciblex cibley set label-color yellow] ]
